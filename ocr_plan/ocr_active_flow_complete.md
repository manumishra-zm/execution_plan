...
flowchart TD
    ...
    subgraph FE["FRONTEND · erpsense-frontend (Next.js)"]
        U["User drag-drop / file picker<br/><i>chat-input.tsx</i><br/>client limits: 30MB · 5 files · 10 pages/PDF"]
...
| Concern | File | Notes |
|---|---|---|
| File picker + client limits | [`src/components/chat/chat-input.tsx`](../../erpsense-frontend/src/components/chat/chat-input.tsx) | 30 MB/file, 5 files, 10 pages/PDF, accepted extensions |
...
- Limits enforced: `MAX_FILE_SIZE=30MB`, `MAX_TOTAL_SIZE=32MB` (Cloud Run cap), `MAX_FILES_PER_SESSION=5`,
...
| Max file size (API) | `30 MB` | `upload.py` |
| Max session size | `32 MB` (Cloud Run) | `upload.py` |
| Max files/session | `5` | `upload.py` |
...
> pages → re‑encoded to **JPEG** → **Vertex AI Gemini** 4‑model fallback (`2.5‑flash → 2.0‑flash → 2.5‑pro →
> 2.0‑flash‑lite`), confidence fixed at **`0.95`**. Post‑processing (structuring, markdown, ERP fields) is
